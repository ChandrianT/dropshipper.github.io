...
class Checkout extends CI_Controller {
	...
	public function customer_login(){
		...
		foreach ($data['trx'] as $row) {
			...
			if ($ress && isset($ress->transaction_status)) {
				$transaction_status = $ress->transaction_status;
				$this->CheckoutModel->update($transaction_status, $row->order_id);

				if ($transaction_status == "settlement") {
					$this->CheckoutModel->updateStatus($transaction_status, $email);
				}
			} else {
				// transaksi tidak ada di midtrans, skip saja
				continue;
			}
	...
	public function membership(){
		...
		foreach ($data['trx'] as $row) {
			...
			if ($ress && isset($ress->transaction_status)) {
				...
				if ($transaction_status == "settlement") {
					$this->CheckoutModel->updateStatus($email);
				}
			} else {
				log_message('error', 'Midtrans membership(): transaksi tidak ditemukan. Order ID: ' . $b);
				continue;
			}
	...
	public function payment(){
		...
		foreach ($data['trx'] as $row) {
			...
			if ($ress && isset($ress->transaction_status)) {
				...
				log_message('error', 'Midtrans payment(): transaksi tidak ditemukan. Order ID: ' . $b);
				continue;
			}
		}
